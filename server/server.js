...
function dealDecks(roomId) {
    ...
    io.to(rooms[roomId][0]).emit("startGame", {
        ...
        playerFranchisePool: creatorReserve,
        aiFranchisePool: maskReserve(joinerReserve),
        gameMode,
    ...
    io.to(rooms[roomId][1]).emit("startGame", {
        ...
        playerFranchisePool: joinerReserve,
        aiFranchisePool: maskReserve(creatorReserve),
        gameMode,
...
io.on("connection", (socket) => {
    ...
    socket.on("playerSwapped", ({ roomId, selectedCandidate }) => {
        ...
        socket.to(roomId).emit("opponentSwapped", candidate);
        emitGameStateUpdate(roomId);
    });
    ...
    socket.on("reconnectRoom", async (data) => {
        ...
        socket.emit("startGame", {
            ...
            playerFranchisePool: role === "creator" ? game.creatorReserve : game.joinerReserve,
            aiFranchisePool: maskReserve(role === "creator" ? game.joinerReserve : game.creatorReserve),
    ...
    socket.on("leaveRoom", ({ roomId }) => {
        ...
        // Notify any remaining client that player has left
...
server.listen(PORT, () => {
    ...
    // Start the automatic stats updater cron job (restricted to March-June 1st week inside cronScheduler)
    initCron();
});